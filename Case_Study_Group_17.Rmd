...
In this section, we import the data for further analysis. We only imported the necessary datasets. This process involves several steps:
...
- **Loading geo dataframe:**
  ...
  - The columns that describe "Gemeinde", "PLZ", Longitude" and "Latitude" are relevant for us

- **Loading data for the heatmap map:**
  - We loaded a Shapefile (.shp) that includes all map information about the municipalities in Germany ("germany_municipalities.shp"). This dataset was downloaded from "opendata-esri-de.opendata.arcgis.com" and was created by the "Federal Agency for Cartography and Geodesy" of Germany.
...
- **Parse to the right date format**
  - Parse the production date of datasets "Fahrzeuge_OEM2_Typ21.csv" and "Fahrzeuge_OEM2_Typ22.csv" in the form: "%d-%m-%Y"
...
- **Prepare the geo dataframe:**
  ...
  - Only keep the needed "Gemeinde", "PLZ", Longitude" and "Latitude" column
...
- **Join the dataframes for the result**
  ...
  - Merge the result with the geo dataframe by the same "Gemeinde" to add the "PLZ", "Longitude" and "Latitude" to the final dataframe.
